...
@AndroidEntryPoint
class ScanWajahFragment : Fragment() {
    companion object {
        const val CAMERA_PERMISSION_REQUEST_CODE = 100
    }
    private val viewModel: AttendanceViewModel by viewModels()
    private lateinit var binding: FragmentScanWajahBinding
    ...
    override fun onCreateView(
        ...
        binding = FragmentScanWajahBinding.inflate(inflater, container, false)
        return binding.root
    }
...
//                            activity.onBackPressed()
                            findNavController().navigate(R.id.action_scanWajahFragment_to_attendanceFragment)

//                            val data = "hasilScanWajah"
//                            val action = ScanWajahFragmentDirections.actionScanWajahFragmentToAttendanceFragment(data)
//                            findNavController().navigate(action)
                        } else {
                            Toast.makeText(requireContext(), "fragment tidak dalam activity", Toast.LENGTH_SHORT).show()
                        }
                    }
                }
            }
